...
import { PaymentModal, PaymentStatusBadge, PrintPaymentModal } from '@/components/payment-components';
...
export default function TrainingPage({ user, onAuthOpen, onNavigate }: TrainingProps) {
  ...
  const [printPaymentModalOpen, setPrintPaymentModalOpen] = useState(false);
  ...
  // === Reset scroll vers le HAUT à chaque changement de vue ou chapitre ===
  // Sans ça, quand l'utilisateur est en bas d'un chapitre et clique sur le
  // chapitre suivant (ou change de vue : catalog → detail → learn → exam),
  // la nouvelle page s'affiche à la même position verticale — il doit
  // remonter manuellement. On force donc le scroll en haut à chaque
  // changement de `view` (catalog/detail/learn/exam/attestation/myAttestations)
  // ET à chaque changement de `currentChapterIdx` (chargement d'un autre
  // chapitre dans la vue 'learn').
  useEffect(() => {
    if (typeof window !== 'undefined') {
      // 'auto' (pas 'smooth') car on veut un reset instantané, pas une animation
      // pendant que le nouveau contenu se charge.
  ...
  // === Ouvrir un chapitre spécifique depuis la vue 'detail' (catalogue des chapitres) ===
  // Permet à l'utilisateur de cliquer sur le titre d'un chapitre dans la liste
  // du cours pour y accéder directement — même si le cours est terminé, l'utilisateur
  // peut revoir n'importe quel chapitre.
  // Pré-initialise le tableau `chapters` avec des stubs basiques (titre + id) pour
  // que la sidebar de la vue 'learn' affiche tous les chapitres, puis remplace le
  // chapitre cliqué par son contenu complet chargé depuis l'API.
  const openChapterFromDetail = async (chapterId: string, index: number) => {
    if (!selectedCourse) return;
    if (!selectedCourse.chapters || selectedCourse.chapters.length === 0) {
      toast.error('Ce cours n\'a pas encore de contenu disponible');
      return;
    ...
    try {
      ...
      if (res.ok) {
        const ch = await res.json();
        // Pré-remplir chapters avec des stubs basiques pour la sidebar
        // (l'utilisateur peut cliquer sur les autres chapitres dans la sidebar
        // pour les charger à la volée via loadChapter).
        const initialChapters: ChapterFull[] = selectedCourse.chapters.map((c, i) => {
          if (i === index) return ch;
          // Stub : juste assez de données pour la sidebar
          return {
            id: c.id, title: c.title, order: c.order, courseId: selectedCourse.id,
            content: '', isCompleted: false, chapterScore: null, questions: [],
          };
        });
  ...
  // Payment modal render — DECLARED HERE (before any view return) to avoid TDZ issue.
  // Previously these were declared AFTER `return null;` (dead code), making the modals
  // never render. Moving them here ensures they are initialized before any view return
  // that references them.
  //
  // onSuccess(paymentMethod) — handler called after the modal's success state.
  // For wallet: refresh selectedCourse from /api/courses (so paymentStatus becomes
  // 'validated' in state) AND auto-trigger POST /api/courses/attestations to generate
  // the CourseAttestation immediately (since wallet = instant validation).
  // For PayPal/bank_transfer: just refresh selectedCourse (paymentStatus stays
  // 'submitted' — admin validation pending).
  const handlePaymentSuccess = useCallback(async (paymentMethod: string) => {
    if (!user || !selectedCourse) return;

    // 1) Re-fetch ALL courses → updates `courses` state (catalog view).
    // We also need to update `selectedCourse` so the DETAIL view's
    // `selectedCourse.enrollment.paymentStatus` reflects the new state
    // (was the cause racine of the stale UI bug — fetchCourses only updated
    // the courses array, not selectedCourse).
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data)) {
          setCourses(data);
          // Find the course that matches selectedCourse and replace it.
          const refreshed = data.find((c: any) => c.id === selectedCourse.id);
          if (refreshed) {
            setSelectedCourse(refreshed);
            // Also update completedChapters from the refreshed enrollment
            if (refreshed.enrollment?.completedChapters) {
              setCompletedChapters(new Set(refreshed.enrollment.completedChapters));
            }
          }
        }
      }
    ...
    // 2) For wallet payment (instant validation) → auto-trigger attestation
    //    generation via POST /api/courses/attestations. The backend will
    //    apply canIssueAttestation() gate (COURSE COMPLETED ≠ PAYMENT VALIDATED
    //    ≠ ATTESTATION ELIGIBLE) — if not eligible yet, it returns 403.
    if (paymentMethod === 'wallet') {
      try {
        const attRes = await fetch('/api/courses/attestations', {
          method: 'POST',
          ...
          body: JSON.stringify({
            ...
            overallScore: selectedCourse.enrollment?.overallScore ?? 0,
          }),
        });
        if (attRes.ok) {
          const att = await attRes.json();
          // Re-fetch attestations list AND switch to attestation view
          try {
            const attListRes = await fetch(`/api/courses/attestations?userId=${user.id}`);
            if (attListRes.ok) {
              const attList = await attListRes.json();
              if (Array.isArray(attList)) setAttestations(attList);
            }
          } catch { /* ignore */ }
          if (att) {
            setCurrentAttestation({
              ...att,
              course: { title: selectedCourse.title, slug: selectedCourse.slug, level: selectedCourse.level, totalHours: selectedCourse.totalHours }
            });
            setView('attestation');
            toast.success('Attestation générée avec succès !');
          }
        } else {
          // Attestation not yet eligible — show user a friendly message
          // but DON'T crash. The payment succeeded; attestation will be
          // generated later when eligibility criteria are met.
          const err = await attRes.json().catch(() => ({ error: 'Erreur inconnue' }));
          if (attRes.status === 403) {
            toast.message('Paiement validé. Attestation en attente d\'éligibilité.', {
              description: err.error || 'Vous devez encore remplir les conditions d\'éligibilité.',
            });
          } else {
            toast.error(err.error || 'Erreur lors de la génération de l\'attestation');
          }
        }
      } catch {
        toast.error('Erreur réseau lors de la génération de l\'attestation');
      }
    }
  }, [user, selectedCourse]);
  ...
  // Print payment modal render (pour l'attestation imprimée 190 MAD — actuellement désactivé).
  // Conservé comme code dormant pour usage futur après migration DB.
  const printPaymentModal = printPaymentModalOpen && selectedCourse?.enrollment ? (
    <PrintPaymentModal
      open={printPaymentModalOpen}
      onOpenChange={setPrintPaymentModalOpen}
      enrollmentId={selectedCourse.enrollment.id}
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          openChapterFromDetail(ch.id, idx);
                        }}
                        className={`w-full text-left flex items-center gap-3 p-3 rounded-lg cursor-pointer transition ${done ? 'bg-emerald-50 border border-emerald-200 hover:border-emerald-400 hover:shadow-md' : 'bg-white border border-slate-200 hover:border-emerald-300 hover:shadow-md'}`}
                        title={`Ouvrir le chapitre : ${ch.title}`}
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`font-medium text-sm ${done ? 'text-emerald-800' : 'text-slate-700 hover:text-emerald-700'}`}>{ch.title}</p>
                ...
                <CardContent className="p-6 space-y-4">
                  ...
                  {!enrolled ? (
                    ...
                    <>
                      ...
                      {allPassed ? (
                        // === FORMATION TERMINÉE + EXAMEN RÉUSSI ===
                        // Affichage des options d'attestation (la règle métier critique).
                        // Aucun prix n'est affiché avant ce moment.
                        selectedCourse.enrollment?.paymentStatus === 'validated' || selectedCourse.enrollment?.paymentStatus === 'not_required' ? (
                          // Paiement déjà validé (ou gratuit) → bouton unique "Obtenir mon attestation"
                        ...
                        ) : selectedCourse.enrollment?.courseOrderIndex === 1 ? (
                          // === FORMATION 01 (courseOrderIndex === 1) → ENTÈREMENT GRATUITE ===
                          // Aucun prix affiché, juste "Obtenir mon attestation"
                          <Button className="w-full" size="lg" variant="outline" onClick={requestAttestation}>
                            <Award className="h-4 w-4 mr-2" /> Obtenir mon attestation
                          </Button>
                        ) : (
                          // === FORMATIONS 02 à 06 (courseOrderIndex >= 2) ===
                          // === WORKFLOW 190 MAD DÉSACTIVÉ — DB n'a pas les colonnes recipient ===
                          // === Bouton unique : attestation numérique 120 MAD uniquement ===
                          <div className="space-y-2">
                            <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-sm text-emerald-800 text-center">
                              🎉 Formation terminée avec succès ! Obtenez votre attestation numérique.
                            </div>
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
            ...
            {examResult.passed && isLastChapter && allPassed && (
              selectedCourse?.enrollment?.paymentStatus === "validated" || selectedCourse?.enrollment?.paymentStatus === "not_required" ? (
                ...
                // Formation 01 : entièrement gratuite → juste "Obtenir mon attestation"
                <Button onClick={requestAttestation} className="bg-emerald-600 hover:bg-emerald-700">
                  <Award className="h-4 w-4 mr-1" /> Obtenir mon attestation
                </Button>
              ) : (
