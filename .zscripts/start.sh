...
if [ -f "./next-service-dist/server.js" ]; then
    ...
    # ============================================================================
    # SÉCURITÉ : Exiger DATABASE_URL en production.
    # Refuser de démarrer pour éviter de servir une DB vide ou de test.
    # La DB officielle est /home/u398373271/hseacademy-data/backups/custum.db
    # ============================================================================
    if [ -z "$DATABASE_URL" ]; then
        echo "❌ DATABASE_URL n'est pas définie."
        echo "   L'application refuse de démarrer pour protéger la DB de production."
        echo "   Configure DATABASE_URL dans le panel Z.ai Build System."
        exit 1
    fi

    echo "🗄️  DATABASE_URL utilisée: $DATABASE_URL"
