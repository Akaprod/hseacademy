...
interface UserProfile {
  ...
  avatar?: string | null;
  // ===== Coordonnées professionnelles CV (ajoutées par LOT 2 — report de last-source) =====
...
export default function ProfilePage({ user, onNavigate, onLogout, initialTab }: ProfilePageProps) {
  ...
  const [cvContactEmail, setCvContactEmail] = useState('');
  const [cvContactLocation, setCvContactLocation] = useState('');
  ...
  useEffect(() => {
    ...
    setCvContactPhone(p.cvContactPhone || p.phoneNormalized || '');
    setCvContactEmail(p.cvContactEmail || '');
    setCvContactLocation(p.cvContactLocation || p.residence || '');
    // CV fields
  ...
  const handleSaveAllCV = async () => {
    ...
    try {
      ...
      await fetch('/api/profile', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ facebook, linkedin, twitter, website, cvContactPhone, cvContactEmail, cvContactLocation }),
      });
            ...
            <div className="space-y-6">
              ...
              <Card className="border-emerald-200 shadow-sm">
                ...
                <CardContent>
                  ...
                  <div>
                    <Label className="text-sm font-semibold text-slate-700 mb-2 block">Coordonnées professionnelles (CV)</Label>
                    <p className="text-xs text-slate-500 mb-3">Ces informations apparaissent dans le bloc identité de votre CV public. Par défaut, votre téléphone et email de compte sont proposés.</p>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div className="space-y-1">
                        <Label htmlFor="cvContactPhone" className="text-xs">Téléphone pro</Label>
                        <Input id="cvContactPhone" value={cvContactPhone} onChange={(e) => setCvContactPhone(e.target.value)} placeholder={profile?.phoneNormalized || '+212 6 XX XX XX XX'} />
                      </div>
                      <div className="space-y-1">
                        <Label htmlFor="cvContactEmail" className="text-xs">Email pro</Label>
                        <Input id="cvContactEmail" type="email" value={cvContactEmail} onChange={(e) => setCvContactEmail(e.target.value)} placeholder={user?.email || 'contact@exemple.com'} />
                      </div>
                      <div className="space-y-1">
                        <Label htmlFor="cvContactLocation" className="text-xs">Localisation pro</Label>
                        <Input id="cvContactLocation" value={cvContactLocation} onChange={(e) => setCvContactLocation(e.target.value)} placeholder={profile?.residence || 'Casablanca, Maroc'} />
                      </div>
                    </div>
                  </div>
                </CardContent>
