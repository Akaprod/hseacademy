...
function MonthYearInput({ value, onChange, placeholder = "MM-AAAA", disabled = false }: {
  ...
  // Guard: s'assurer que value est toujours une string (jamais null/undefined)
  ...
  // Value format: "MM-YYYY" — guard contre valeur vide ou malformée
  ...
  return (
    <div className="flex gap-2">
      <Select value={month} onValueChange={(m) => onChange(year ? `${m}-${year}` : m)} disabled={disabled}>
        <SelectTrigger className="flex-1"><SelectValue placeholder="Mois" /></SelectTrigger>
        <SelectContent>
          {months.map(m => <SelectItem key={m.v} value={m.v}>{m.l}</SelectItem>)}
        </SelectContent>
      </Select>
      <Select value={year} onValueChange={(y) => onChange(month ? `${month}-${y}` : y)} disabled={disabled}>
...
// ============================================================================
function SectionProfilPro({ initial, onSave }: { initial: string; onSave: (v: string) => void }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    const ok = await patchCV({ cvProfessionalProfile: value });
    setSaving(false);
    if (ok) {
  ...
  return (
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="pb-3">
            ...
            <button onClick={() => setOpen(!open)} className="flex items-center justify-between w-full text-left">
              <CardTitle className="flex items-center gap-2 text-emerald-700 text-base">
                <FileText className="h-5 w-5" /> Profil professionnel
              </CardTitle>
        ...
        {open && (
          <CardContent>
            <Label htmlFor="cv-professional">Présentation professionnelle</Label>
            <Textarea
              id="cv-professional"
              value={value}
              ...
              rows={6}
              maxLength={1500}
              placeholder="Présentez votre parcours, votre spécialisation, vos compétences principales et vos objectifs professionnels."
              className="mt-1"
            />
            <p className="text-xs text-slate-400 mt-1">{value.length}/1500 caractères. Présentation détaillée (distincte de la bio courte).</p>
...
function SectionAdditionalInfo({ initial, onSave }: { initial: AdditionalInfo; onSave: (v: AdditionalInfo) => void }) {
  // Guard : s'assurer que tous les champs existent (même si initial est {} ou partiel)
  const safeInitial: AdditionalInfo = {
    drivingLicenses: initial?.drivingLicenses || [],
    drivingLicenseOther: initial?.drivingLicenseOther || '',
    mobility: initial?.mobility || '',
    availability: initial?.availability || '',
    professionalProjects: initial?.professionalProjects || '',
    volunteerExperiences: initial?.volunteerExperiences || [],
    interests: initial?.interests || [],
  };
  const [info, setInfo] = useState<AdditionalInfo>(safeInitial);
  const [open, setOpen] = useState(false);
  const [saving, setSaving] = useState(false);
...
export default function CVSections({ profile, onChanged }: CVSectionsProps) {
  ...
  const [professionalProfile, setProfessionalProfile] = useState(profile?.cvProfessionalProfile || '');
  ...
  const [additionalInfo, setAdditionalInfo] = useState<AdditionalInfo>(parseJsonSafe(profile?.cvAdditionalInfo, {
    drivingLicenses: [], drivingLicenseOther: '', mobility: '', availability: '',
    professionalProjects: '', volunteerExperiences: [], interests: [],
  }));

  // Re-charger quand le profile change
  useEffect(() => {
    setProfessionalProfile(profile?.cvProfessionalProfile || '');
    ...
    setAdditionalInfo(parseJsonSafe(profile?.cvAdditionalInfo, {
      drivingLicenses: [], drivingLicenseOther: '', mobility: '', availability: '',
      professionalProjects: '', volunteerExperiences: [], interests: [],
    }));
  }, [profile]);
  ...
  return (
    <div className="space-y-4">
      {/* SectionProfilPro retiré — doublon UX avec cvBio de la CARD 3 "Identité professionnelle".
          Le champ DB cvProfessionalProfile est conservé mais non édité ici (CV public affiche cvBio).
          Bug 1 — corrections finales V1. */}
