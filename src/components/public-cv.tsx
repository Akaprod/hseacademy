...
import {
  ...
  Download, Link2, Share2, MessageCircle, ChevronDown,
} from 'lucide-react';
import {
...
/**
 * Extrait l'identifiant affichable d'une URL de réseau social ou de site web.
 *
 * - LinkedIn /in/<id>        → <id>
 * - Facebook /<id>           → <id>
 * - X / Twitter /<id>        → @<id>
 * - Instagram /<id>          → @<id>
 * - HSE Academy /@<id>       → @<id>
 * - Site web classique       → domaine.com (sans www, sans chemin)
 *
 * L'URL d'origine reste cliquable (href intact).
 */
function extractSocialIdentifier(url: string, network: 'linkedin' | 'facebook' | 'twitter' | 'website'): string {
  if (!url) return '';
  // Nettoyage : retirer protocol + www pour parser le host/path
  let urlStr = url.trim();
  ...
  try {
    ...
    return url; // fallback : URL brute si parsing échoue
  }

  switch (network) {
    case 'linkedin': {
      // URL typique : https://www.linkedin.com/in/farid-ouamrhar
      // → extraire le segment après /in/, /company/, /pub/, /school/, etc.
      const m = url.match(/linkedin\.com\/(?:in|company|pub|school|profile)\/([^/?#]+)/i);
      return m ? decodeURIComponent(m[1]) : (path || 'LinkedIn');
    }
    case 'facebook': {
      // URL typique : https://www.facebook.com/farid.ouamrhar
      // → identifier = premier segment du path (sans query/hash)
      // Cas particulier : https://www.facebook.com/profile.php?id=1625080054
      // → pas de username, on extrait le param ?id= à la place
      const id = path.split('/')[0];
      if (id && id !== 'profile.php') return decodeURIComponent(id);
      // Fallback pour profile.php?id=XXX → afficher l'ID numérique
      const m = url.match(/[?&]id=(\d+)/);
      return m ? m[1] : (host || 'Facebook');
    }
    case 'twitter': {
      // URL typique : https://x.com/akaprod ou https://twitter.com/akaprod
      // → @<id>
      const id = path.split('/')[0];
      return id ? '@' + decodeURIComponent(id) : (host || '@Twitter');
    }
    case 'website': {
      // Site web classique : retourner le domaine (sans www, sans chemin)
      // Ex : https://faridouamrhar.com → faridouamrhar.com
      //      https://www.example.com/page → example.com
      //      https://hseacademy.online/@farid → @farid (identifiant du profil)
      if (/^hseacademy\.online$/i.test(host) && /^@/.test(path)) {
        return '@' + path.replace(/^@/, '').split('/')[0];
      }
      // Cas Instagram (au cas où le champ "website" contienne une URL insta)
      if (/^instagram\.com$/i.test(host)) {
        const id = path.split('/')[0];
        return id ? '@' + decodeURIComponent(id) : host;
      }
...
function Footer({ username }: { username: string }) {
  const cvUrl = `https://hseacademy.online/@${username}`;
  const shareText = `Découvrez mon CV professionnel sur HSE Academy ${cvUrl}`;
  const shareTextEncoded = encodeURIComponent(shareText);
  const cvUrlEncoded = encodeURIComponent(cvUrl);
  ...
  const handleCopyLink = async () => {
    if (typeof navigator !== 'undefined' && navigator.clipboard) {
      try {
        ...
        if ((window as any).sonerToast) {
          (window as any).sonerToast.success('Lien copié dans le presse-papier');
        }
      } catch (e) {
        // Fallback silencieux
      }
  ...
  const shareOptions = [
    ...
    },
    {
      label: 'Facebook',
      icon: Facebook,
      href: `https://www.facebook.com/sharer/sharer.php?u=${cvUrlEncoded}`,
      color: 'text-blue-600',
    },
    {
      label: 'Twitter / X',
      icon: Twitter,
      href: `https://twitter.com/intent/tweet?text=${shareTextEncoded}&url=${cvUrlEncoded}`,
      color: 'text-sky-500',
    },
    {
      label: 'LinkedIn',
      icon: Linkedin,
      href: `https://www.linkedin.com/sharing/share-offsite/?url=${cvUrlEncoded}`,
      color: 'text-blue-700',
    },
  ];
  ...
  return (
    <div className="text-center pt-6 pb-8 space-y-4">
      {/* === 3 BOUTONS SEULEMENT === : Télécharger PDF | Partager (menu) | Copier le lien */}
      <div className="flex justify-center gap-3 flex-wrap print:hidden">
        {/* 1. Télécharger PDF — window.print() */}
        <button
          onClick={handlePrint}
        ...
        {/* 2. Partager — menu déroulant avec WhatsApp / Facebook / Twitter / LinkedIn */}
        <DropdownMenu>
        ...
        {/* 3. Copier le lien — navigator.clipboard */}
        <button
          onClick={handleCopyLink}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-200 hover:bg-slate-300 text-slate-700 text-sm font-medium transition-colors"
        >
      ...
      {/* Ligne d'attribution navigateur (cachée à l'impression) */}
      <div className="text-xs text-slate-300 print:hidden">
        <a href={cvUrl} className="hover:underline">hseacademy.online/@{username}</a>
        <span className="mx-2">•</span>
        <span>CV hébergé sur HSE Academy</span>
      </div>

      {/* Pied de page d'impression — visible uniquement à l'impression PDF.
          Placé en fin de CV → apparaît en bas de la dernière page.
          Username dynamique selon le propriétaire du CV. */}
      <div className="hidden print:block cv-print-footer">
        <span>hseacademy.online/@{username}</span>
        <span className="mx-2">•</span>
        <span>CV généré par HSEAcademy.com</span>
      </div>
    </div>
  );
}
        ...
        <div className="text-white p-8 space-y-6" style={{ backgroundColor: accent }}>
          ...
          {(profile.linkedin || profile.facebook || profile.twitter || profile.website) && (
            <div style={{ borderTop: `1px solid ${primary}40`, paddingTop: '14pt' }}>
              ...
              <div className="space-y-2">
                {profile.linkedin && <a href={profile.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm opacity-90 hover:opacity-100" title={profile.linkedin}><Linkedin className="h-4 w-4 shrink-0" /> <span className="truncate max-w-[180px]">{extractSocialIdentifier(profile.linkedin, 'linkedin')}</span></a>}
                {profile.facebook && <a href={profile.facebook} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm opacity-90 hover:opacity-100" title={profile.facebook}><Facebook className="h-4 w-4 shrink-0" /> <span className="truncate max-w-[180px]">{extractSocialIdentifier(profile.facebook, 'facebook')}</span></a>}
                {profile.twitter && <a href={profile.twitter} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm opacity-90 hover:opacity-100" title={profile.twitter}><Twitter className="h-4 w-4 shrink-0" /> <span className="truncate max-w-[180px]">{extractSocialIdentifier(profile.twitter, 'twitter')}</span></a>}
                {profile.website && <a href={profile.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm opacity-90 hover:opacity-100" title={profile.website}><Globe className="h-4 w-4 shrink-0" /> <span className="truncate max-w-[180px]">{extractSocialIdentifier(profile.website, 'website')}</span></a>}
              </div>
            </div>
          )}
      ...
      <div className="max-w-3xl mx-auto py-10 px-6">
        ...
        <div className="text-center space-y-4 pb-8" style={{ marginBottom: '14pt', borderBottom: `2px solid ${primary}20` }}>
          ...
          <div className="flex justify-center gap-4 flex-wrap pt-2">
            {profile.linkedin && <a href={profile.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-sm hover:opacity-70" style={{ color: primary }} title={profile.linkedin}><Linkedin className="h-5 w-5" /> <span className="truncate max-w-[200px]">{extractSocialIdentifier(profile.linkedin, 'linkedin')}</span></a>}
            {profile.facebook && <a href={profile.facebook} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-sm hover:opacity-70" style={{ color: primary }} title={profile.facebook}><Facebook className="h-5 w-5" /> <span className="truncate max-w-[200px]">{extractSocialIdentifier(profile.facebook, 'facebook')}</span></a>}
            {profile.twitter && <a href={profile.twitter} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-sm hover:opacity-70" style={{ color: primary }} title={profile.twitter}><Twitter className="h-5 w-5" /> <span className="truncate max-w-[200px]">{extractSocialIdentifier(profile.twitter, 'twitter')}</span></a>}
            {profile.website && <a href={profile.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-sm hover:opacity-70" style={{ color: primary }} title={profile.website}><Globe className="h-5 w-5" /> <span className="truncate max-w-[200px]">{extractSocialIdentifier(profile.website, 'website')}</span></a>}
          </div>
        </div>
            ...
            {(profile.linkedin || profile.facebook || profile.twitter || profile.website) && (
              <div style={{ marginBottom: '14pt' }}>
                ...
                <div className="space-y-2 text-sm text-slate-600">
                  {profile.linkedin && <a href={profile.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:opacity-70" style={{ color: primary }} title={profile.linkedin}><Linkedin className="h-4 w-4 shrink-0" /> <span className="truncate max-w-[180px]">{extractSocialIdentifier(profile.linkedin, 'linkedin')}</span></a>}
                  {profile.facebook && <a href={profile.facebook} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:opacity-70" style={{ color: primary }} title={profile.facebook}><Facebook className="h-4 w-4 shrink-0" /> <span className="truncate max-w-[180px]">{extractSocialIdentifier(profile.facebook, 'facebook')}</span></a>}
                  {profile.twitter && <a href={profile.twitter} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:opacity-70" style={{ color: primary }} title={profile.twitter}><Twitter className="h-4 w-4 shrink-0" /> <span className="truncate max-w-[180px]">{extractSocialIdentifier(profile.twitter, 'twitter')}</span></a>}
                  {profile.website && <a href={profile.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:opacity-70" style={{ color: primary }} title={profile.website}><Globe className="h-4 w-4 shrink-0" /> <span className="truncate max-w-[180px]">{extractSocialIdentifier(profile.website, 'website')}</span></a>}
                </div>
              </div>
            )}
